  ...
  <v-dialog :value="value" max-width="900px" persistent scrollable>
    <v-card>
      ...
      <v-card-text>
        <v-container>
          <v-row>
            ...
            <v-col cols="3">{{viewAllocation.PurposeTitle}} {{viewAllocation.Id}}</v-col>
            ...
            <v-col cols="3">{{viewAllocation.CreatedAt | simpleDateTime}}</v-col>
            <v-col cols="3"><strong>Letzte Veränderung:</strong> </v-col>
            <v-col cols="3">{{viewAllocation.LastModified | simpleDateTime}}</v-col>
            ...
            <v-col cols="3"> {{contactUserName(viewAllocation.ReferencePerson)}}</v-col>
          ...
          <v-row v-show="moveEdit">
            <v-col cols="12"><h2 :class="{'valid-range': timesInvalid}">Neuer Zeitraum</h2></v-col>
            <v-col cols="3"><strong>Von:</strong></v-col>
            <v-col cols="3"><input type="datetime-local" v-model="editFrom" :class="{'invalid-date': !editFrom}"/></v-col>
            <v-col cols="3"><strong>Bis:</strong></v-col>
            <v-col cols="3"><input type="datetime-local" v-model="editTo" :class="{'invalid-date': !editTo}"/></v-col>
          </v-row>
        </v-container>
        ...
        <v-row v-for="(i,idx) in possibleCollisions" v-bind:key="idx+'cols'">
            <v-col cols="4">{{i.From | simpleDateTime}} - {{i.To | simpleDateTime}}</v-col>
      ...
      <v-card-actions>
        ...
        <v-btn color="green darken-1" :disabled="moveEdit" text @click="acknowledge"><v-icon v-html="'done'"></v-icon> Bestätigen</v-btn>
        <v-btn color="red darken-1" :disabled="moveEdit" text @click="reject"><v-icon v-html="'close'"></v-icon> Ablehnen</v-btn>
        <v-btn color="gray darken-1" :disabled="timesInvalid && moveEdit" text @click="move"><v-icon v-html="moveEdit ? 'done':'create'"></v-icon> Verschieben</v-btn>
        <v-btn color="orange darken-1" text @click="cancel"><v-icon>close</v-icon> Abbrechen</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
...
export default class AcknowledgeView extends Vue {
  ...
  public get possibleCollisions (): Allocations[] {
    ...
    const rValues = Allocations.query().withAll()
      ...
      .where((a: any) => {
          ...
          const rVal = a.Id !== id && (aTo >= start && aTo <= end) ||
  ...
  public contactUserName (id: number): string { // acts as filter, because it causes errors as filter
    return (this.ContactUsers.find((w: ContactUser) => w.Id === id) || { Title: '' }).Title
  }
  ...
  public async move () {
    ...
    const changedAllocation: AllocationModel = { ...this.viewAllocation, From: this.editFrom, To: this.editTo }
  ...
  public async saveStatus (task: AllocationModel, status: number) {
    ...
    Allocations.update({ where: task.Id, data: { From: task.From, To: task.To, Status: status } })
    this.$emit('input', false)
  }
}
</script>
