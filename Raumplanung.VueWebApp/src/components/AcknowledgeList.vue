<template>
  ...
  <v-data-table v-if="hasItems"
    ...
    >
      ...
      <template v-slot:item.action="{ item }"><v-btn @click="openDialog(item.Id)">Bearbeiten</v-btn></template>
...
export default class AcknowledgeList extends Vue {
  ...
  public openDialog (id: number) {
    const viewA = Allocations.query().withAll().where('Id', id).first() as any
    if (viewA == null) return
    this.dialog = true
    this.viewAllocation = { 
      ...viewA, RessourceTitle: viewA.Ressource.Name,
      PurposeTitle: viewA.Purpose.Title, ContactTel: viewA.Purpose.ContactPhone,
      Description: viewA.Purpose.Description, Notices: viewA.Purpose.Notes 
    }
  }
