...
//                {id: 7, name: 'Raum 1', startAfter: 9 * 60, duration: 2 * 60},
    ...
    methods: {
        ...
        zoomAxisX (dir) {
            ...
            if(dir === 'in') {
                this.updateAxisX(getDatePlusDuration(start, diff), getDatePlusDuration(end, -diff));
            }
            else if(dir === 'out') {
                this.updateAxisX(getDatePlusDuration(start, -diff), getDatePlusDuration(end, diff));
            }
        ...
        updateAxisX(start, end, animDuration = 200) {
            ...
            this.elContainer.selectAll('.slot')
                .transition(trans)
                .attr('transform', d => `translate(${ this.scaleX(this.getStart(d)) }, ${ this.scaleY(d.name) })`);
            
            this.elContainer.selectAll('.slot')
                .transition(trans)
                .select('.zone')
                .attr('width', d => this.scaleX(this.getEnd(d)) - this.scaleX(this.getStart(d)));

            this.elContainer.selectAll('.slot')
                .transition(trans)
                .select('.handlerRight')
                .attr('x', d => (this.scaleX(this.getEnd(d)) - this.scaleX(this.getStart(d))) - 5);
                
            this.elMain.select('.dateStartAll')
                .transition(trans)
        ...
        updateStartDate (date) {
            ...
            document.getElementById('startDate').innerHTML = formatDate(this.dateStartAll);
            ...
            const [start, end] = this.scaleX.domain();
            this.updateAxisX(getDatePlusDuration(start, diffDuration), getDatePlusDuration(end, diffDuration), 0);
        ...
        updateData () {
            const self = this
            const updateSlots = this.elContainer.selectAll('.slot').data(self.data, d => d.id);
            ...
            const entry = enterSlots.append('g')
                ...
                .attr('name', d => d.name)
                .attr('transform', d => `translate(${ self.scaleX(self.getStart(d)) }, ${ self.scaleY(d.name) })`);
            
            entry.append('rect')
                .attr('class', 'zone')
                .attr('x', 0)
                .attr('y', -slotHeight/2)
                .attr('width', d => self.scaleX(self.getEnd(d)) - self.scaleX(self.getStart(d)))
                .attr('height', slotHeight)
                .attr('cursor', 'move')
            ...
            entry.append('rect')
                    ...
                    .attr('x', d => (self.scaleX(self.getEnd(d)) - self.scaleX(self.getStart(d))) - 5)
                    .attr('y', d => -slotHeight/2)
                    .attr('width', 10)
                    .attr('height', slotHeight)
                    .attr('cursor', 'ew-resize')
            ...
            function dragZoneProgress(slot) {
                ...
                const newValue = self.calculateNewValue(slot, tempStartDate, slot.duration, 'move');
                slot.startAfter = newValue.startAfter;
                slot.duration = newValue.duration;
                
                // change g.slot translation
                d3.select(this.parentNode)
                    .attr('transform', `translate(${ self.scaleX(self.getStart(slot)) }, ${ self.scaleY(slot.name) })`);
            ...
            function dragLeftProgress(slot) {
                ...
                const newValue = self.calculateNewValue(slot, tempStartDate, tempDuration, 'left');
                slot.startAfter = newValue.startAfter;
                slot.duration = newValue.duration;
                ...
                d3.select(this.parentNode)
                    .select('.zone')
                    .attr('width', self.scaleX(self.getEnd(slot)) - self.scaleX(self.getStart(slot)))
            ...
            if(slot) {
                ...
                document.getElementById('selection').innerHTML = `
                    <div style="background: grey;">
                        <h3>NAME: ${ this.currentSlot.name } (id=${ this.currentSlot.id })<br>
